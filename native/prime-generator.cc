...
std::vector<std::string> generatePrimesThreads(int threadCount, int primeCount, const std::string startValue, bool format, bool log) {
    ...
    auto workerFunction = [&](int count, const std::string& start, bool format, bool log) {
        // Generate primes
    ...
    for (int i = 0; i < threadCount; ++i) {
        ...
        threads.push_back(std::thread(workerFunction, primeCount / threadCount, updatedStartValue, format, log));
